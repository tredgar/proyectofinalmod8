...
app.post('/api/productos', (request, response) => {
  ...
  if (!nombre || !precio) {
    return response.status(400).json({ mensaje: 'Nombre y precio son obligatorios' });
  }
  ...
  response.status(201).json({ mensaje: 'Producto creado exitosamente', producto: nuevoProducto });
});
